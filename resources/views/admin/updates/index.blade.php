...
<div class="card shadow mb-4">
  <div class="card-header">
    <h6 class="m-0 font-weight-bold text-primary">{{ trans('changelog::admin.settings.title') }}</h6>
  </div>
  <div class="card-body">
    <form action="{{ route('changelog.admin.settings.save') }}" method="POST">
      @csrf

      <div class="form-group">
        <div class="form-group">
          <label for="perpage">{{ trans('changelog::admin.settings.perpage') }}</label>
          <input class="form-control" id="perpage" name="perpage" type="number"
            value="{{ old('perpage', setting('changelog.perpage')) }}" required="required">
        </div>
      </div>
...
<div class="card shadow mb-4">
  ...
  <div class="card-body">
    ...
    <a href="{{ route('changelog.admin.categories.create') }}" class="btn btn-primary"><i class="fas fa-plus"></i>
...
<div class="card shadow mb-4">
  ...
  <div class="card-body">
    ...
    <div class="alert alert-danger mb-0"><i
        class="fas fa-exclamation-circle pr-2"></i>{{ trans('changelog::admin.categories.nothing') }}</div>
    @endif
  </div>
</div>
@endcan
